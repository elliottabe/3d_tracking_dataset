...
(``fruitfly_v2_3_ik/fruitfly_v2_3_ik.xml`` in the shared body-model checkout,
generated by ``scripts/models/build_v2_3_ik_model.py`` and gitignored there)
rather than the shared model itself: the shared repo's model keeps its 8 adhesion
...
# Sibling checkout of Brunton-Lab/fruitfly_body_models == paths.body_model_dir.
BODY = REPO.parent / 'fruitfly_body_models'
SHARED_V2_3_XML = BODY / 'fruitfly_v2.3' / 'fruitfly_muscles_warp.xml'
V2_3_XML = BODY / 'fruitfly_v2_3_ik' / 'fruitfly_v2_3_ik.xml'

# The derived model is built on demand, not committed, so a fresh checkout has
# nothing to check. Skip loudly rather than failing on a missing artifact.
requires_derived = pytest.mark.skipif(
    not V2_3_XML.exists(),
    reason=f'{V2_3_XML} missing -- run scripts/models/build_v2_3_ik_model.py')
requires_shared = pytest.mark.skipif(
    not SHARED_V2_3_XML.exists(), reason=f'{SHARED_V2_3_XML} missing')
