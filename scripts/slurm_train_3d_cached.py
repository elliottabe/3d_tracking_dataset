...
Usage:
    ...
    python scripts/slurm_train_3d_cached.py --run-name myrun

    # Resume / continue a specific run (same ckpt dir):
    python scripts/slurm_train_3d_cached.py --run-name cached3d_v3_8gpu_20260620

    # Pass Hydra overrides (forwarded to both compose and job body):
    python scripts/slurm_train_3d_cached.py --run-name myrun train.total_steps=30000 train.sharpen=3

    # See the script without submitting:
    python scripts/slurm_train_3d_cached.py --run-name t --dry-run train.total_steps=30000 train.sharpen=3
"""
...
[ -f {cache_dir}/train_meta.json ] || python -u scripts/precompute_repro_cache.py paths={paths} cache.split=train
# Precompute cache if absent (val split)
[ -f {cache_dir}/val_meta.json ] || python -u scripts/precompute_repro_cache.py paths={paths} cache.split=val
python -u -m jarvis_jax.train.train_3d_cached \\
    run_id={run_name} train=cached3d paths={paths}{overrides_str}
"""
...
def main():
    ...
    run_name = args.run_name or f"cached3d_v3_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"Cache   : {cache_dir} (precompute-if-absent)")
    if passthrough:
