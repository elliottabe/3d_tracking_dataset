...
**`fly_id` normalization (added after the smoke test).** The shard script derives
a session tag from `--video_folder` as its last two path components
(`Session1/<rec>`) and keeps only bout-CSV rows whose `fly_id` equals that tag
(`predict3D_multianimal_shard.py:90-94`, mirrored in the child's `parse_bouts`).
The current CSVs, however, were regenerated by a later "unify bouts" step that
stamped `fly_id = Session1_bouts_04172026/<rec>` — so nothing matched and the
first smoke test aborted with `No bouts ... for session Session1/2026_04_02_16_39_56`
(0 of 18). Fix: the manifest generator writes a **normalized** copy of each CSV
under `session1_bouts_normalized/<rec>/courtship_bouts_unified_summary.csv` with
`fly_id` rewritten to the derived tag (all other columns preserved), and the
manifest's second column points at the normalized CSV. The shared shard script
and the original CSVs are left untouched. This reproduces the `fly_id` format the
earlier successful run consumed. So the manifest's second column is a
`session1_bouts_normalized/…` path, not the raw `Session1_bouts_04172026/…` CSV.
