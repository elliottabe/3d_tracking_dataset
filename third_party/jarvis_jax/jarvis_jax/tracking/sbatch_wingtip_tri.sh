...
#
# POC: triangulate the SAM-mask wing tip across cameras and compare to GT vs the
# compressed V2VNet prediction.
set -x
source ~/.bashrc
...
cd "$PKG"
python -u -m jarvis_jax.tracking.demo_wingtip_triangulation \
    --root "$ROOT" --aux "$WORK/cse_labels_val_M300.npz" \
    --cache-dir "$WORK/cache_350" --ckpt-dir "$RUNS/cse_v2v350/ckpt" \
    --mesh "$MESH" --num-joints 350 --n-kp 50 --n 4 --corridor 12 \
    --out "$WORK/viz/wingtip_triangulation.png"
echo "WINGTIP TRI DONE"
