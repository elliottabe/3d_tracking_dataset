...
This guide explains how to process all Predictions_3D folders in the  dataset using the batch processing scripts.
...
│   ├── _bouts_summary.csv     # Bout info with fly_id ({dataset}_bouts_summary.csv)
...
# /data2/users/eabe/datasets/Johnson_lab//Predictions_3D_20260202-171900
...
```bash
cd stac-mjx
python run_stac_fly_model.py paths=workstation dataset= anatomy=v1 \
...
In the base directory (`/data2/users/eabe/datasets/Johnson_lab//Data_analysis/Testing/`):
...
ls -lh /data2/users/eabe/datasets/Johnson_lab//Predictions_*/preprocessed_bout_*.h5

# List STAC outputs
ls -lh /data2/users/eabe/datasets/Johnson_lab//Predictions_*/Fruitfly_ik_*.h5

# List postprocessing outputs
ls -lh /data2/users/eabe/datasets/Johnson_lab//Predictions_*/ik_output_*.h5
```
...
**Solution:** Ensure your `{dataset}_bouts_summary.csv` (e.g. `_bouts_summary.csv`) has a `fly_id` column. It should look like:
```csv
...
python scripts/preprocess_keypoints_for_ik.py \
    paths=workstation dataset= anatomy=v1 \
    paths.data_dir=/data2/users/eabe/datasets/Johnson_lab//Predictions_3D_20260202-171900
```
...
Configuration:
  Anatomy: v1
  Dataset: 
  Base directory: /data2/users/eabe/datasets/Johnson_lab/
